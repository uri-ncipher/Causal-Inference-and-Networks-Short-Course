...
```{r}
nodes$employment <- factor(
  nodes$employment,
  levels = c(0, 1),
  labels = c("unemployed", "employed")
)
network::set.vertex.attribute(my_network.ergm, "education",as.numeric(nodes$education))
network::set.vertex.attribute(my_network.ergm, "employment",as.character(nodes$employement))
 ...
 *Simple Bernoulli (“Erdos/Renyi”) model*. Begin with a simple model, containing only the total number of edges in the network: 'edges' - when included in an ERGM its coefficient controls the overall density of the network.
...
```{r}
summary(my.ergm.bern) # more generally, for any such model, you can look at the $g(y)$ statistic for this model
...
```
  ...
  One important step is to check the diagnostic of the model.
