...
#adjusted models
#Risk Difference
# Binomial Log model did not converge - fit with GEE log-Poisson to get a robust 
...
#Risk Ratio
# Binomial Identity model did not converge - fit with GEE Normal-identity to get a robust 
# estimator of the variance
# model_rd <- glm(
...
make_continuous_table <- function(data, 
                                  treatment_var, 
                                  cont_vars, 
                                  digits = 2, 
                                  show_sd = TRUE) {
...
#Fit treatment model
mylogit <- glm(treatment ~ age10 + posneg + date + share + education + employment,
               data = nodes, 
               family = "binomial")
...
gcomp_results$estimate <- round(gcomp_results$estimate, 3)
gcomp_results$lower_95_ci <- round(gcomp_results$lower_95_ci, 3)
gcomp_results$upper_95_ci <- round(gcomp_results$upper_95_ci, 3)

gcomp_results
#save confidence intervals
gcomp_rd_ci <- c(gcomp_results$lower_95_ci[3],gcomp_results$upper_95_ci[3])
gcomp_rr_ci <- c(gcomp_results$lower_95_ci[4],gcomp_results$upper_95_ci[4])
...
```{r}
#Code to output results to Latex and Forest Plot (optional)
...
# Keep only causal estimators for the forest plot
...
# Colors matched to the slide
uri_teal <- "#349B9B"
uri_navy <- "#0B2E63"
uri_gray <- "#B8B8B8"
uri_light_gray <- "#F2F4F5"

estimator_colors <- c(
  "Unadjusted" = uri_navy,
  "Covariate adjusted" = uri_teal,
  "IPW" = "#5CB8B2",
  "G-computation" = "#2F8F8F",
  "DR1" = "#0B2E63",
  "DR2" = "#6AA6A6"
)

forest_rr <- forest_data %>%
  filter(Measure == "Risk ratio") %>%
  ggplot(aes(x = Estimate, y = Estimator, color = Estimator)) +
  geom_vline(xintercept = 1, linetype = "dashed", color = uri_navy, linewidth = 0.5) +
  geom_errorbarh(aes(xmin = LCL, xmax = UCL), height = 0.15, linewidth = 0.8) +
  geom_point(size = 2.2) +
  ...
  theme(
    plot.title = element_text(color = uri_navy, face = "bold"),
    axis.text.y = element_text(color = uri_navy, size = 14),
    axis.title.x = element_text(color = uri_navy),
    panel.grid.major.y = element_blank(),
    panel.grid.minor = element_blank(),
    panel.grid.major.x = element_line(color = uri_light_gray),
    legend.position = "none"
  )

forest_rd <- forest_data %>%
  filter(Measure == "Risk difference") %>%
  ggplot(aes(x = Estimate, y = Estimator, color = Estimator)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = uri_navy, linewidth = 0.5) +
  geom_errorbarh(aes(xmin = LCL, xmax = UCL), height = 0.15, linewidth = 0.8) +
  geom_point(size = 2.2) +
  scale_color_manual(values = estimator_colors, drop = FALSE) +
  labs(
...
ggsave(
  filename = "effect_estimates_forest_plot_rr.pdf",
  plot = forest_rr,
  width = 8,
  height = 7
)
...
ggsave(
  filename = "effect_estimates_forest_plot_rd.pdf",
  plot = forest_rd,
  width = 8,
  height = 7
)
